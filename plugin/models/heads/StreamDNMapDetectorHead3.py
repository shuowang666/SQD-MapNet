...
class StreamDNMapDetectorHead3(nn.Module):
    ...
    def __init__(self, 
        ...
        self.dn_cfg = dn_cfg
        self.dn_stream_cfg = dn_stream_cfg
    ...
    def stream_denoise(self, gt_vecs_list, gt_labels_list_x, cur_gt_vecs_list, cur_gt_labels_list_x, device, prev2curr_matrix_list, prop_query_embedding):
        ...
        if self.dn_stream_cfg is not None:
            self.dn_generator = CdnQueryGenerator(**self.dn_stream_cfg)
    ...
    def adaptive_select(self, prev_lines, prev_labels, lines, labels, prev2curr_matrix_list):
        ...
        for i, prev_l in enumerate(prev_labels):
            ...
            id_tmp = [-1 for _ in range(len(prev_l))]
            cost_tmp = [100 for _ in range(len(prev_l))]
            cur_all_label = labels[i]

            # 将prev gt转到当前帧下
            prev_ref_pts = prev_lines[i][:, 0, :].reshape(-1, 20, 2)
            denormed_ref_pts = prev_ref_pts * self.roi_size + self.origin # (num_prop, num_pts, 2)
            denormed_ref_pts = torch.cat([
                denormed_ref_pts,
                denormed_ref_pts.new_zeros((denormed_ref_pts.size(0), self.num_points, 1)), # z-axis
            ...
            cur_real_gt = lines[i][:, 0, :].reshape(-1, 20, 2) * self.roi_size + self.origin

            for j in range(len(cur_all_label)):
                cost = [100 for _ in range(len(prev_l))]
                for k in range(len(prev_l)):
                    if cur_all_label[j] == prev_l[k]:
                        cost[k] = self.chamfer_distance(cur_real_gt[j], curr_ref_pts[k, :, :2])
                value_min = cost[0]
                idx_min = 0
                for p in range(1, len(cost)):
                    if cost[p] < value_min:
                        value_min = cost[p]
                        idx_min = p
                # import ipdb; ipdb.set_trace()
    ...
    def forward_train(self, bev_features, img_metas, gts):
        ...
        last_id_list = self.adaptive_select(copy.deepcopy(last_gt_list), copy.deepcopy(last_label_list),
        ...
        if self.iter >= self.dn_iter:
            if self.stream_dn:
                ...
                stream_dn_query = torch.cat((stream_dn_query, dn_query), 1)
                stream_denoise_refers = torch.cat((stream_denoise_refers, denoise_refers), 1)
                tgt_size = stream_dn_meta['pad_size'] + dn_meta['pad_size'] + self.num_queries
                attn_masks = torch.ones(tgt_size, tgt_size).to(self_attn_mask.device) < 0
                attn_masks[stream_dn_meta['pad_size']:, stream_dn_meta['pad_size']:] = self_attn_mask
                attn_masks[stream_dn_meta['pad_size']:, :stream_dn_meta['pad_size']] = True
                attn_masks[:stream_dn_meta['pad_size'], :stream_dn_meta['pad_size']] = stream_self_attn_mask[:stream_dn_meta['pad_size'], :stream_dn_meta['pad_size']]
                attn_masks[:stream_dn_meta['pad_size'], stream_dn_meta['pad_size']:stream_dn_meta['pad_size']+dn_meta['pad_size']] = True

                # 对dn query信息进行融合，并对dn_meta进行合并
            else:
                stream_dn_query, stream_self_attn_mask, stream_dn_meta, stream_denoise_refers \
                            = self.denoise(
                            copy.deepcopy(gts[0]['lines']), copy.deepcopy(gts[0]['labels']), 
        ...
        for i, (queries) in enumerate(inter_queries):
            reg_points = inter_references[i][:, dn_num+dn_num2:] # (bs, num_q, num_points, 2)
            bs = reg_points.shape[0]
            reg_points = reg_points.view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)

            scores = self.cls_branches[i](queries[:, dn_num+dn_num2:]) # (bs, num_q, num_classes)
        ...
        if dn_num != 0:
            ...
            loss_dn_dict = self.loss_dn(gts=copy.deepcopy(gts[0]), last_gts=copy.deepcopy(last_gt_list), 
                                            last_ids=last_id_list, last_labels=copy.deepcopy(last_label_list), \
        ...
        if dn_num2 != 0:
            ...
            outputs_dn = []
            for i, (queries) in enumerate(inter_queries):
                reg_points = inter_references[i][:, dn_num:dn_num+dn_num2] # (bs, num_q, num_points, 2)
                bs = reg_points.shape[0]
                reg_points = reg_points.view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)

                scores = self.cls_branches[i](queries[:, dn_num:dn_num+dn_num2]) # (bs, num_q, num_classes)
            ...
            loss_dn2_dict = self.loss_dn_normal(gts=copy.deepcopy(gts), preds=outputs_dn, dn_meta=dn_meta)
            loss_dict.update(loss_dn2_dict)
        ...
        if self.streaming_query:
            ...
            for i in range(bs):
                _lines = lines[i]
                _queries = inter_queries[-1][i][dn_num+dn_num2:]
                _scores = scores[i]
            ...
            self.stream_dn_target_memory.update(gts[0]['lines'], ids=None, labels=gts[0]['labels'], img_metas=img_metas)
            ...
            if ids is not None and i in dn_meta['stream_valid']:
                ...
                for j, id_per in enumerate(ids):
                    ...
                    if id_per == -1: #or len(pos)==0:
                        ...
                        tmp_labels.append(gts['labels'][i][id_per])
    ...
    def loss_dn_normal_single(self,
                    preds,
                    gts,
                    dn_meta,
                    gt_bboxes_ignore_list=None,
        ...
        cls_labels = torch.cat(gt_labels_list, dim=0).repeat(group) # (bs*num_q, )
        # cls_weights = torch.ones_like(cls_labels) # (bs*num_q, )
        cls_weights = torch.cat(dn_meta['loss_weight']).repeat(group)
        # cls_weights = dn_meta['loss_weight']

        gt_pts_list = [gt_line[:, 0, :] for gt_line in gts['lines']]
        gt_lines = torch.cat(gt_pts_list, 0).repeat(group, 1)
        line_weights = torch.cat(dn_meta['loss_weight']).repeat(group)[:, None].repeat(1, gt_lines.size(1))
        # line_weights = dn_meta['loss_weight'][:, None].repeat(1, gt_lines.size(1))
        ...
        loss_dict = dict(
            normal_dn_cls=loss_cls,
            normal_dn_reg=loss_reg,
        )
    ...
    def loss_dn_normal(self,
        ...
        # multi_apply(self.loss_dn_single, preds, gts, dn_meta_tmp, reduction=reduction)
        for i in range(len(preds)):
            losses.append(self.loss_dn_normal_single(preds[i], gts[i], dn_meta, reduction=reduction))
        # import ipdb; ipdb.set_trace()
