...
class DNMapDetectorHead(nn.Module):
    ...
    def _init_embedding(self):
        ...
        if self.dn_cfg is not None:
            ...
            self.label_embedding = nn.Embedding(self.cls_out_channels, self.embed_dims//2)
    ...
    def denoise(self, gt_vecs_list, gt_labels_list, device):
        ...
        if self.dn_cfg is not None:
            ...
            # Choice One
            # pe_bbox = self.dn_bbox(input_query_bbox)
            # pe_pts = self.dn_pts(input_query_pts)
            # dn_query_pos = (pe_pts + pe_bbox.unsqueeze(2)).flatten(1, 2)
            # dn_query_content = dn_query_content.unsqueeze(2).repeat(1, 1, self.num_pts_per_gt_vec, 1).flatten(1, 2)
            """
            # Choice Two，先进行DAB-DETR编码，再MLP    
            pe_bbox = self.dn_bbox(gen_sineembed_for_position(input_query_bbox, self.embed_dims//2))
            pe_pts = self.dn_pts(gen_sineembed_for_position(input_query_pts.flatten(1, 2), self.embed_dims//2))
            # 用sin编码order
            # order_embedding = SinePositionalEncoding(torch.ones(self.num_pts_per_vec).to(device), self.embed_dims, normalize=True)
            # dn_query_pos = (pe_pts.reshape(pe_pts.size(0), pe_bbox.size(1), 20, pe_pts.size(2)) + pe_bbox.unsqueeze(2) + order_embedding[None, None]).flatten(1, 2)
            dn_query_pos = (pe_pts.reshape(pe_pts.size(0), pe_bbox.size(1), 20, pe_pts.size(2)) + pe_bbox.unsqueeze(2) + self.order_embedding.weight[None, None]).flatten(1, 2)
            # dn_query_pos = (pe_bbox.unsqueeze(2) + self.order_embedding.weight[None, None]).flatten(1, 2)
            # dn_query_pos = (pe_bbox.unsqueeze(2) + order_embedding[None, None]).flatten(1, 2)
            dn_query_content = dn_query_content.unsqueeze(2).repeat(1, 1, self.num_pts_per_gt_vec, 1).flatten(1, 2)   # torch.Size([2, 720, 256])
            ...
            # Choice Four，去掉bbox embed
            # import ipdb; ipdb.set_trace()
            pe_pts = self.dn_pts(gen_sineembed_for_position(denoise_refers.flatten(1, 2), self.embed_dims//4))
            # order_embedding = SinePositionalEncoding(torch.ones(self.num_pts_per_vec).to(device), self.embed_dims, normalize=True)
            # 没有 instance embed
            # dn_query_pos = (pe_pts.reshape(pe_pts.size(0), -1, 20, pe_pts.size(2)) + order_embedding[None, None]).flatten(1, 2)
            # dn_query_pos = (pe_pts.reshape(pe_pts.size(0), -1, 20, pe_pts.size(2)) + self.order_embedding.weight[None, None]).flatten(1, 2)
            # dn_query_pos = pe_pts.reshape(pe_pts.size(0), -1, 20, pe_pts.size(2))
            # order_embedding = SinePositionalEncoding(torch.ones(self.num_pts_per_vec).to(device), self.embed_dims, normalize=True)
            # 加入 instance embed
            pe_pts = pe_pts.reshape(pe_pts.size(0), -1, 20, pe_pts.size(2)).flatten(2)
            # instance = pe_pts.max(2)[0]
            # instance = self.instance_convert(pe_pts.max(2)[0])
            # instance = self.instance_convert(pe_pts.mean(2))
            # dn_query_pos = (pe_pts + instance.unsqueeze(2) + self.order_convert(order_embedding[None, None])).flatten(1, 2)
            # dn_query_pos = (pe_pts + instance.unsqueeze(2) + self.order_embedding.weight[None, None]).flatten(1, 2)
            ...
            # return torch.cat((query[..., :self.embed_dims-1], torch.zeros(query.size(0), query.size(1), 1).to(query.device)), -1), \
            #         query[..., self.embed_dims-1:], attn_mask, dn_meta, denoise_refers
            # return torch.zeros_like(dn_query_content), dn_query_pos, attn_mask, dn_meta, denoise_refers
            return query, attn_mask, dn_meta, denoise_refers
            # return dn_query_content, dn_query_pos, attn_mask, dn_meta, denoise_refers
    ...
    def forward_train(self, bev_features, img_metas, gts):
        ...
        # import ipdb; ipdb.set_trace()
        # print("is_first_frame_list:", is_first_frame_list)
        ...
        if self.dn_cfg is not None:
            ...
            # )
            # self_attn_mask[dn_num+self.num_vec_one2one :, 0+dn_num : self.num_vec_one2one+dn_num] = True
            # self_attn_mask[0+dn_num : self.num_vec_one2one+dn_num, self.num_vec_one2one+dn_num :,] = True
        ...
        inter_queries, init_reference, inter_references = self.transformer(
            ...
            dn_query=dn_query,
            dn_refer=denoise_refers,
            attn_masks=self_attn_mask,
        )

        outputs = []
        for i, (queries) in enumerate(inter_queries):
            reg_points = inter_references[i][:, dn_meta['pad_size']:] # (bs, num_q, num_points, 2)
            bs = reg_points.shape[0]
            reg_points = reg_points.view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)

            scores = self.cls_branches[i](queries[:, dn_meta['pad_size']:]) # (bs, num_q, num_classes)
        ...
        loss_dn_dict = self.loss_dn(gts=gts, preds=outputs_dn, dn_meta=dn_meta)
        ...
        if self.streaming_query:
            ...
            for i in range(bs):
                _lines = lines[i]
                _queries = inter_queries[-1][i][dn_meta['pad_size']:]
                _scores = scores[i]
    ...
    def loss_dn(self,
        ...
        for i in range(len(preds)):
            losses.append(self.loss_dn_single(preds[i], gts[i], dn_meta, reduction=reduction))
        # import ipdb; ipdb.set_trace()
